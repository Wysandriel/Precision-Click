...
玩家需要在限定時間內點擊正確目標，並避開陷阱。點擊正確目標可以得分，失誤會扣分並中斷連擊。

### 目標與陷阱

- 白色目標：一般目標，點中即可得分。
- 金色目標：獎勵目標，得分更高，並額外增加 2 秒。
- 紅色炸彈：危險陷阱，扣分、扣生命，並中斷連擊。
- 紫色假目標：誘餌陷阱，扣分並中斷連擊，但不扣生命。
- 藍色冰凍：冰凍陷阱，扣分、扣生命、扣時間，並短暫鎖住操作。
...
- 連擊越高，倍率越高。
- 點錯空白處會扣分並中斷連擊。
- 漏掉白色或金色目標會扣分、扣生命並中斷連擊。
- 高評級需要高分、高準確率、快速反應與穩定連擊。

### 模式說明

- Classic：30 秒標準挑戰，適合熟悉規則。
- Rush：45 秒高速模式，目標節奏更快，分數倍率較高。
- Survival：60 秒生存模式，考驗穩定度與失誤控制。
...
- 修復暫停後恢復時，目標壽命計時器可能失效的問題。
- 修復遊戲中打開排行榜或規則頁，返回後目標殘留或不重新生成的問題。
- 修復從結算畫面打開排行榜/規則後，關閉時直接跳回主選單的問題。
- 修復排行榜評級徽章過大造成版面擠壓的問題。
- 加強按鈕事件綁定防呆，降低 HTML 與 JS 不同步時造成整個遊戲無法執行的風險。


### 開始遊戲誤扣分修復

- 修復按下「開始遊戲」時，該次點擊可能被遊戲場地誤判為點錯空白處，導致一開局就扣分的問題。
- 新增 450ms 開局輸入保護。
- 選單、規則、排行榜、結算畫面的點擊不會再冒泡到遊戲場地。
